...
import { startOfZonedDay, zonedParts, zonedTimeToUtc } from "../lib/schedule";
import type { WeeklyHours } from "../lib/enums";

/**
 * Seed data: real Lausanne venues, so the first run looks like the product rather
 * than like a fixture. Coordinates are hand-entered to roughly building level and
 * should be treated as approximate — nothing here has been through a geocoder.
 *
 * Dated events are positioned relative to the moment you seed, so there is always
 * something on today and something on this weekend whenever you reset.
 */
...
type SeedActivity = Omit<
  Parameters<typeof prisma.activity.create>[0]["data"],
  "submitter" | "reports"
>;
...
async function main() {
  ...
  // Two regulars whose reports have already earned them some credibility, plus a
  // newcomer — enough for all three trust badges to be visible on first load.
